...
_TYPE_EXPORTS = {"DenseSectorSample", "Sample", "TrainingStepResult"}
_TASK_API_EXPORTS = {
    "evaluate",
    "load_config",
...
def __getattr__(name: str) -> Any:
    if name in _DATASET_EXPORTS:
        from tasks.tissue_doppler import dataset

        return getattr(dataset, name)
    if name in _TYPE_EXPORTS:
        from tasks.tissue_doppler import types
    ...
    if name in _TASK_API_EXPORTS:
        ...
        api = bind_task_api("tissue_doppler")
